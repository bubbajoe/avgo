...
func videoInputFirstPacket() (pkt *avgo.Packet, err error) {
	if global.pkt != nil {
	...
	if fc, err = videoInputFormatContext(); err != nil {
		err = fmt.Errorf("avgo_test: getting input format context failed")
		return
	}

	pkt = avgo.AllocPacket()
	if pkt == nil {
		err = errors.New("avgo_test: pkt is nil")
		return
	}
...
func TestPacket(t *testing.T) {
	...
	require.True(t, pkt1.Flags().Has(avgo.PacketFlagKey))
	...
	pkt2.SetDts(1)
	pkt2.SetDuration(2)
	pkt2.SetFlags(avgo.NewPacketFlags(3))
	pkt2.SetPos(4)
	pkt2.SetPts(5)
	pkt2.SetSize(6)
	...
	require.Equal(t, avgo.NewPacketFlags(3), pkt2.Flags())
	...
	pkt3.SetDuration(20)
	pkt3.SetPts(30)
	pkt3.RescaleTs(avgo.NewRational(1, 10), avgo.NewRational(1, 1))
	require.Equal(t, int64(1), pkt3.Dts())
	require.Equal(t, int64(2), pkt3.Duration())
	require.Equal(t, int64(3), pkt3.Pts())

	pkt4 := avgo.AllocPacket()
	...
	pkt5 := avgo.AllocPacket()
	require.NotNil(t, pkt5)
	defer pkt5.Free()
	b = []byte{1, 2, 3, 4}
	require.NoError(t, pkt5.AddSideData(avgo.PacketSideDataTypeAudioServiceType, b))
	require.Equal(t, b, pkt5.SideData(avgo.PacketSideDataTypeAudioServiceType))

	pkt6 := avgo.AllocPacket()
